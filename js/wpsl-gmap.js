...
jQuery(document).ready(function ($) {
    ...
    if ($(".wpsl-mobile").length) {
      ...
      $(".wpsl-dropdown").each(function (index) {
        ...
        $this.$dropdownWrap = $this
          .wrap("<div class='wpsl-dropdown'></div>")
          .parent();
        $this.$dropdownElem = $("<div><ul/></div>").appendTo(
          $this.$dropdownWrap
        );
        $this.$selectedVal = $this.val();
        $this.$dropdown = $this.$dropdownElem.find("ul");
  ...
  function selectedEmptyInput() {
    ...
    if ($("span.wpsl-selected-item").length) {
      ...
      if (
        !parseInt($("#wpsl-radius-dropdown").val()) ||
        !parseInt($("#wpsl-results-dropdown").val())
      ) {
        emptySelect = true;
      }
    }
  ...
  $("#wpsl-search-input").change(function () {
    if (!checkMobileUserAgent()) {
      ...
      if ($("#wpsl-radius span.wpsl-selected-item").attr("data-value")) {
        radiusSelected = parseInt(
          $("#wpsl-radius span.wpsl-selected-item").attr("data-value")
        );
      } else {
